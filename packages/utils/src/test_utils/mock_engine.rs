pub static COMPONENT_SQUARE_BYTES: &[u8] =
    include_bytes!("../../../../examples/build/components/square.wasm");

pub static COMPONENT_ECHO_DATA_BYTES: &[u8] =
    include_bytes!("../../../../examples/build/components/echo_data.wasm");

pub static COMPONENT_KV_STORE_BYTES: &[u8] =
    include_bytes!("../../../../examples/build/components/kv_store.wasm");

pub static COMPONENT_PERMISSIONS_BYTES: &[u8] =
    include_bytes!("../../../../examples/build/components/permissions.wasm");
