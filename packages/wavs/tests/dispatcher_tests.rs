...
use crate::wavs_systems::mock_submissions::SUBMISSION_TIMEOUT;
...
fn dispatcher_node_status() {
    ...
    let contract_address = rand_address_cosmos();
    let actions = vec![mock_real_trigger_action(
        &service_id,
        &workflow_id,
        &contract_address,
        &SquareIn::new(3),
    ...
    ctx.rt.block_on(async {
        ...
        dispatcher
            .services
            .update_node_service_status(service_id.clone(), false);
        dispatcher
            .trigger_manager
            .send_dispatcher_commands(actions.iter().cloned().map(DispatcherCommand::Trigger))
            .await
            .unwrap();
    });
    ...
    assert_eq!(dispatcher.submission_manager.get_message_count(), 0);

    ctx.rt.block_on(async {
        dispatcher
            .services
            .update_node_service_status(service_id.clone(), true);
        dispatcher
            .trigger_manager
            .send_dispatcher_commands(actions.into_iter().map(DispatcherCommand::Trigger))
            .await
    ...
    // Ensure we have a message after enabling the service
    wait_for_submission_messages(&dispatcher.submission_manager, 1, None).unwrap();
}
