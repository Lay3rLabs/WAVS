...
use utils::{eth_client::EthSigningClient, storage::db::{DBError, RedbStorage, Table, JSON}};
use wavs_types::{Aggregator, ChainName, EthereumContractSubmission, EventId, Packet, PacketRoute, Service, Submit};
...
// key is EventId
const PACKET_QUEUES: Table<&[u8], JSON<PacketQueue>> =
    Table::new("packet_queues");

// key is PacketRoute 
const DESTINATIONS: Table<&str, JSON<Destination>> =
    Table::new("destinations");
...
impl HttpState {
    pub fn new(config: Config) -> anyhow::Result<Self> {
        ...
        Ok(Self { config, storage, eth_clients })
    }
    ...
    pub async fn get_eth_client(&self, chain_name: &ChainName) -> anyhow::Result<EthSigningClient> {
        ...
        self.eth_clients.write().unwrap().insert(chain_name.clone(), eth_client.clone());
    ...
    pub fn get_packet_queue(&self, event_id: &EventId) -> anyhow::Result<PacketQueue> {
        match self
            .storage
            .get(PACKET_QUEUES, event_id.as_ref())?
        {
        ...
        for (workflow_id, workflow) in service.workflows.iter() {
            if matches!(workflow.submit, Submit::Aggregator{..}) {
                match &workflow.aggregator {
                    None => {
                        bail!("No aggregator set for service {} workflow {}", service.id, workflow_id);
                    },
                    Some(Aggregator::Ethereum(submission)) => {
                        let key = packet_route_key(&PacketRoute { service_id: service.id.clone(), workflow_id: workflow_id.clone() });
                            ...
                            .is_none()
                        {
                            ...
                            self.storage
                                .set(DESTINATIONS, &key, &Destination::Eth(submission.clone()))?;
                        } else {
