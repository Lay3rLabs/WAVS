...
3. `MATIC_LOG_LEVEL` env var override (see [example wavs.toml comments](./wavs.toml) for more info on overrides)
4. `RUST_LOG` env var
...
Also, it is less forgiving of spaces in between directives (e.g. `MATIC_LOG_LEVEL="info, wavs=debug"` is ok, but `RUST_LOG="info, wavs=debug"` is not. Must be `RUST_LOG="info,wavs=debug"`.
